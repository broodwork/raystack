...
from raystack.template import Context, Engine
from raystack.utils import archive
from raystack.utils.http import parse_header_parameters
from raystack.utils.version import get_docs_version


class TemplateCommand(BaseCommand):
    """
    Copy either a Raystack application layout template or a Raystack project
    layout template into the specified directory.

    :param style: A color style object (see raystack.core.management.color).
    ...
    def handle(self, app_or_project, name, target=None, **options):
        ...
        context = Context(
            {
                **options,
                ...
                "raystack_version": raystack.__version__,
            },
            autoescape=False,
        )
        ...
        for root, dirs, files in os.walk(template_dir):
            ...
            for filename in files:
                ...
                # accidentally render Raystack templates files
    ...
    def handle_template(self, template, subdir):
        ...
        Use raystack.__path__[0] as the default because the Raystack install
        directory isn't known.
        """
        if template is None:
            return os.path.join(raystack.__path__[0], "conf", subdir)
        else:
    ...
    def download(self, url):
        ...
        prefix = "raystack_%s_template_" % self.app_or_project
        ...
        opener = build_opener()
        opener.addheaders = [("User-Agent", f"Raystack/{raystack.__version__}")]
        try:
